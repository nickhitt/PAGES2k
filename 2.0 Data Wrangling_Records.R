...
headers <- names(all_records[[2]]) ## pulling out list names

data <- data.frame(all_records[[2]][[1]]) # pulling first record
for (i in 2:length(all_records[[2]])){ #pulling all lists into DF
  data[,i] <- all_records[[2]][[i]]
}
